...
func (c *Conn) StartTLS(config *tls.Config) error {
	...
	// Validate config early to avoid the footgun of tls.Client(nil)
	if config == nil {
		return fmt.Errorf("tls: nil config passed to StartTLS")
	}
	// clone so we don't mutate caller's config
	cfg := config.Clone()
	if cfg.ServerName == "" && !cfg.InsecureSkipVerify {
		// require explicit ServerName unless caller intentionally disabled verification
		return fmt.Errorf("tls: ServerName must be set in tls.Config for StartTLS (or set InsecureSkipVerify)")
	}
	...
	// set a short deadline for the TLS handshake to avoid hangs on pre-TLS garbage
	// (caller can/should call SetDeadline/SetReadDeadline later for long ops)
	_ = c.conn.SetReadDeadline(time.Now().Add(5 * time.Second))
	_ = c.conn.SetWriteDeadline(time.Now().Add(5 * time.Second))
	defer func() {
		// clear deadlines after handshake attempt; ignore errors
	...
	// Force handshake now so verification errors surface in StartTLS
	if err := tlsConn.Handshake(); err != nil {
		// ensure we close tlsConn (which closes underlying) to avoid leaking conn
		_ = tlsConn.Close()
		return fmt.Errorf("tls: handshake failed: %w", err)
	}

	// success: replace underlying connection with tls.Conn
	c.conn = tlsConn
	return nil
}
